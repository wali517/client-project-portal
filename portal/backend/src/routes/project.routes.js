...
router.post('/', authorize(PERMISSIONS.PROJECT_CREATE), validate({ body: createProjectSchema }), projectController.createProject);
router.get('/', authorize(PERMISSIONS.PROJECT_READ), validate({ query: listProjectsQuery }), projectController.listProjects);
router.get('/:id', authorize(PERMISSIONS.PROJECT_READ), validate({ params: idParam }), projectController.getProject);
router.patch(
  '/:id',
...
// Alias kept for the documented endpoint list.
router.post(
  '/:id/revision',
  authorize(PERMISSIONS.PROJECT_REVISION_CREATE),
  validate({ params: idParam, body: revisionSchema }),
