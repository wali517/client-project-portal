...
export const assignStaff = asyncHandler(async (req, res) => {
  const assignments = await projectService.assignStaff(
    req.params.id,
    req.body.staffIds,
    req.user,
  );
...
export const unassignStaff = asyncHandler(async (req, res) => {
  const assignment = await projectService.unassignStaff(
    req.params.id,
    req.params.staffId,
    req.user,
  );
...
export const submitWork = asyncHandler(async (req, res) => {
  const project = await projectService.submitWork(
    req.params.id,
    req.body,
    req.user,
  );
...
export const reviewWork = asyncHandler(async (req, res) => {
  const data = await projectService.adminReview(
    req.params.id,
    req.body,
    req.user,
  );
...
export const clientApprove = asyncHandler(async (req, res) => {
  const project = await projectService.clientApprove(
    req.params.id,
    req.body,
    req.user,
  );
...
export const addFeedback = asyncHandler(async (req, res) => {
  const project = await projectService.addClientFeedback(
    req.params.id,
    req.body.feedback,
    req.user,
  );
...
export const deleteProject = asyncHandler(async (req, res) => {
  const snapshot = await projectService.permanentlyDeleteProject(
    req.params.id,
    req.user,
  );
  return sendSuccess(res, {
...
export const listMessages = asyncHandler(async (req, res) => {
  const { items, pagination } = await messageService.listProjectMessages(
    req.params.id,
    req.query,
    req.user,
  );
