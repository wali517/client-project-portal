...
export const saveUploadedFiles = async ({
  ...
  let project = null;
  if (requestId)
    request = await loadRequestForUser(requestId, user, { populate: false });
  if (projectId)
    project = await loadProjectForUser(projectId, user, { populate: false });
  ...
  for (const file of files) {
    ...
    const doc = await File.create({
      ...
      category:
        category ||
        (request
          ? FILE_CATEGORY.REQUEST_ATTACHMENT
          : FILE_CATEGORY.PROJECT_ATTACHMENT),
    ...
    try {
      ...
      if (err.statusCode === 404) {
        if (
          ...
          !sameId(file.uploadedBy?._id || file.uploadedBy, user._id)
        ) {
...
export const getFileStreamPath = async (fileId, user) => {
  ...
  try {
    ...
    if (file.mimeType?.startsWith("image/")) {
      ...
      return {
        file: { ...rawFile, mimeType: "image/svg+xml" },
    ...
    return {
      file: { ...rawFile, mimeType: "text/plain" },
      buffer: Buffer.from(`File content for ${file.originalName}`),
    };
  }
};
...
export const listFiles = async ({ requestId, projectId, category }, user) => {
  ...
  if (projectId) {
    const project = await loadProjectForUser(projectId, user, {
      populate: false,
    });
    filter.project = project._id;
  } else if (requestId) {
    const request = await loadRequestForUser(requestId, user, {
      populate: false,
    });
