...
const RequestDetails = ({ request, showClient = false }) => {
  ...
  const showRejectionReason = (isAdmin || isClient) && Boolean(request.rejectionReason);
  const filteredAdminNotes = (request.adminNotes || []).filter(
    (note) => !request.rejectionReason || note.note?.trim() !== request.rejectionReason?.trim()
  );
  const showAdminNotes = isAdmin && filteredAdminNotes.length > 0;
      ...
      <CardBody className="space-y-5">
        ...
        {showRejectionReason && (
          <div className="rounded-lg border border-rose-100 bg-rose-50 px-4 py-3">
            <h3 className="text-sm font-semibold text-rose-800">Rejected Notes</h3>
        ...
        {showAdminNotes && (
          <div>
            <h3 className="mb-2 text-sm font-semibold text-ink-900">Related Notes</h3>
